...
```{r}
ggplot(
    filter(summary_data, env_cond=="const-a"),
    aes(
...
```{r}
ggplot(
    filter(summary_data, env_cond=="const-b"),
    aes(
      x=MAX_HEAD_COPY_COST,
      y=dom_match_score_env_b,
      fill=MAX_HEAD_COPY_COST
    )
  ) +
  geom_flat_violin(
...
ggplot(
    time_series_data,
    # filter(time_series_data, env_cond=="const-a" & COPY_MUT_PROB=="0.001" ),
    aes(
      x=update,
