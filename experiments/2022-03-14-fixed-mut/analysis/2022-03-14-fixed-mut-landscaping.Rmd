...
```{r}
ls2_task_toggle_data_loc <- paste0(working_directory, "data/mutant_task_toggle_distribution_step-2.csv")
...
ls2_task_toggle_data$rep_id <- mapply(
  ls2_task_toggle_simplify_replicate_id,
  ls2_task_toggle_data$env_condition,
  ls2_task_toggle_data$COPY_MUT_PROB,
...
ggsave(
  paste0(working_directory, "plots/", "total_npmi_step-2_bymut.pdf")
)
```

## Total NPMI broken down by task pair
...
ggplot(
    filter(
      ...
      COPY_MUT_PROB==m & env_condition%in%e
    ),
    aes(
      x=task_2_id,
      y=npmi,
...
ggsave(
  paste0(working_directory, "plots/", "total_npmi_pairwise_step-2.pdf")
)
```
...
ggsave(
  paste0(working_directory, "plots/", "num_tasks_toggled_dist_step-2.pdf"),
  width=15,
  height=18
)
```
...
fig_num_mutants_that_flip_by_mut_rate <-
  ggplot(
    filter(
      ls2_run_summary_data,
      COPY_MUT_PROB%in%focal_mut_rates & env_condition=="env-cycling_rate-300"
    ),
    aes(
      x=COPY_MUT_PROB,
      y=num_viable_mutants_flip_env
