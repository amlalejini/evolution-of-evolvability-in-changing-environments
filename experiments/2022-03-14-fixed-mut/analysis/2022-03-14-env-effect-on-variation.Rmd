...
ls2_run_summary_data$total_abs_npmi <- ls2_run_summary_data$expected_npmi + ls2_run_summary_data$unexpected_npmi
...
```{r}
kruskal.test(
  formula=prop_viable~env_type,
  data=ls2_run_summary_data
)
```
...
```{r}
kruskal.test(
  formula=num_unique_viable_phenotypes~env_type,
  data=ls2_run_summary_data
)

pairwise.wilcox.test(
  x = ls2_run_summary_data$num_unique_viable_phenotypes,
  g = ls2_run_summary_data$env_type,
...
```{r}
kruskal.test(
  formula=entropy_viable_phenotypes~env_type,
  data=ls2_run_summary_data
)

pairwise.wilcox.test(
  x = ls2_run_summary_data$entropy_viable_phenotypes,
  g = ls2_run_summary_data$env_type,
  p.adjust.method = "bonferroni",
  exact=FALSE
)
...
### Average match score change toward ENV-B
...
ggsave(
  paste0(plot_directory, "avg_match_chg_toward_env_b_step-2.pdf")
)
```
...
```{r}
kruskal.test(
  formula=num_viable_mutants_gain_tasks~env_type,
  data=ls2_run_summary_data
)

pairwise.wilcox.test(
  x = ls2_run_summary_data$num_viable_mutants_gain_tasks,
  g = ls2_run_summary_data$env_type,
  p.adjust.method = "bonferroni",
  exact=FALSE
)
...
### Viable mutants that toggle one task
...
```{r}
toggle_one_task_data <- filter(
      ls2_task_toggle_data,
      (num_tasks_toggled==1)
    )
kruskal.test(
  formula=prop_of_viable~env_type,
  data=toggle_one_task_data
)

pairwise.wilcox.test(
  x = toggle_one_task_data$prop_of_viable,
  g = toggle_one_task_data$env_type,
  p.adjust.method = "bonferroni",
  exact=FALSE
)
```
...
```{r}
kruskal.test(
  formula=total_npmi~env_type,
  data=ls2_run_summary_data
)

pairwise.wilcox.test(
  x = ls2_run_summary_data$total_npmi,
  g = ls2_run_summary_data$env_type,
  p.adjust.method = "bonferroni",
  exact=FALSE
)
...
# ggsave(
#   paste0(plot_directory, "total_abs_npmi_step-2.pdf")
# )
```
...
ggsave(
  paste0(plot_directory, "unexpected_npmi_step-2.pdf")
)
```
...
ggsave(
  paste0(plot_directory, "expected_npmi_step-2.pdf")
)
```
...
ggsave(
  paste0(plot_directory, "npmi_matrices_step-2.pdf"),
  height=15,
  width=12
)
```
...
#### Cyclic environment results in more mutants that can flip phenotype
...
ggsave(
  filename=paste0(plot_directory, "2022-03-14-mutants-fip-env-figure.pdf"),
  plot=fig_num_mutants_that_flip,
  width=12,
  height=4
  # base_height=6
  # base_asp=2
