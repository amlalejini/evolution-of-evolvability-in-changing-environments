"""
This script generates (and analyzes) all pairwise knockouts for each genotype in a target .dat file
"""

import os, argparse, subprocess, itertools, copy
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate + analyze all pairwise knockouts genotypes in a given input file."
    )
    ...
    parser.add_argument("--output", type=str, default="pairwise-knockouts.csv", help="Name of the final output file (dumped in run directory).")
    ...
    for gi in range(len(data)):
        ...
        # knockouts = [[list(sequence) for j in range(len(sequence))] for i in range(len(sequence))]
        knockout_positions = list(itertools.combinations_with_replacement([i for i in range(len(sequence))], 2))
        knockout_positions_lookup = {i+1:knockout_positions[i] for i in range(len(knockout_positions))}
        knockout_positions_lookup[0] = None
        # NOTE - might need to increment i by 1 to account for prepending original sequence?

        knockouts = []
        for pos in knockout_positions:
            knockouts.append(list(sequence))
            knockouts[-1][pos[0]] = inst_map["nop-X"]
            knockouts[-1][pos[1]] = inst_map["nop-X"]
        ...
        output_fname_str = f"pairwise-knockouts/knockouts_id-{gi}.dat"
        ...
        ko_dat_file_info.append({
            "gi": gi,
            "tree_depth": tree_depth,
            "sequence_len": len(sequence),
            "update_born": update_born,
            "knockout_positions_lookup": copy.deepcopy(knockout_positions_lookup)
        })

    # Write knockouts avida analyze file.
    ko_analyze_fname = "analyze_pairwise-knockouts.cfg"
    ...
    avida_cmd = f"./avida {avida_args} -def INST_SET {inst_set_abs_path} -set ANALYZE_FILE {ko_analyze_fname} -a > pairwise-knockouts.log"
    ...
    for ko_i in range(len(ko_dat_files)):
        ...
        ko_data[0]["ko_pos_0"] = "-1"
        ko_data[0]["ko_pos_1"] = "-1"

        for i in range(1, len(ko_data) ):
            ko_data[i]["ko_pos_0"] = info["knockout_positions_lookup"][i][0]
            ko_data[i]["ko_pos_1"] = info["knockout_positions_lookup"][i][1]
    ...
    if do_cleanup:
        ...
        subprocess.run(f"rmdir {os.path.join(run_dir, 'data', 'pairwise-knockouts')}", shell=True)
